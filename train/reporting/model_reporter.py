...
class ModelReporter:
    ...
    def _setup_directories(self):
        ...
        self.current_report_dir = os.path.join(
            self.base_output_dir, folder_name)
        os.makedirs(self.current_report_dir, exist_ok=True)
        self.report_txt_path = os.path.join(
            self.current_report_dir, "summary.txt")
    ...
    def plot_top_2_features_boundary(self):
        ...
        sorted_imps = sorted(
            imps.items(), key=lambda item: item[1], reverse=True)
        ...
        # Tworzy czystą kopię np. SVM, RandomForest, itd.
        clf_2d = clone(classifier)
        ...
        try:
            ...
            print(
                f"[INFO] Nie udało się wytrenować modelu 2D (może model nie obsługuje sparse?): {
                    e
                }"
            )
            return
        ...
        if self.current_report_dir:
            ...
            save_path = os.path.join(
                self.current_report_dir, "decision_boundary_2d.png"
            )
    ...
    def plot_confusion_matrix(self):
        ...
        # Zapis do pliku
        save_path = os.path.join(
            self.current_report_dir, "confusion_matrix.png")
        plt.savefig(save_path)
        plt.close()
        self._log(f"Zapisano macierz pomyłek: {save_path}")

    def plot_confusion_matrix_with_0_as_few_classes(
        self, external_dataset, main_class_name="Klasa główna"
    ):
        y_pred = self.wrapper.predict(self.X_test)
        ...
        merged_df = pd.merge(
            test_df, external_dataset, on="question", how="left"
        ).fillna(0)

        rest_categories = [
            col for col in external_dataset.columns if col != "question"]
        ...
        save_path = os.path.join(
            self.current_report_dir, "prediction_vs_true_class_correlation.png"
        )
        plt.savefig(save_path)
        plt.close()
        self._log(
            f"Zapisano wykres korelacji predykcji z klasami: {save_path}")
    ...
    def run_cross_validation(self, k=5):
        ...
        for fold_idx, (train_index, val_index) in enumerate(kf.split(self.X)):
            ...
            # Obliczanie metryk
            # Używamy macro, aby każda klasa miała takie samo znaczenie
            ...
            self._log(
                f"Fold {
                    fold_idx + 1}: Acc={acc:.4f}, Prec={prec:.4f}, Rec={rec:.4f}, F1={f1:.4f}"
            )
        ...
        for metric_name, values in metrics.items():
            ...
            self._log(f"{metric_name.capitalize()}: {
    ...
    def save_feature_importance(self):
        ...
        if imps:
            ...
            for k, v in sorted_imps:
                val_as_float = float(v) if np.ndim(
                    v) == 0 or np.size(v) == 1 else v
    ...
    def plot_confidence_distribution(self):
        ...
        save_path = os.path.join(
            self.current_report_dir, "confidence_distr_plot.png")
        plt.savefig(save_path)
    ...
    def save_model_and_datasets(self):
        ...
        self._log(
            f"Zapisano model i zbiór danych w {
                self.current_report_dir}/test_set.csv, "
    ...
    def plot_learning_curve(self, cv=5, n_points=5):
        ...
        plt.plot(
            train_sizes, train_mean, "o-", color="#e74c3c", label="Wynik treningowy"
        )
        plt.plot(
            train_sizes, test_mean, "o-", color="#2ecc71", label="Wynik walidacji (CV)"
        )
        ...
        if self.current_report_dir:
            save_path = os.path.join(
                self.current_report_dir, "learning_curve.png")
            plt.savefig(save_path)
            plt.close()
    ...
    def generate_report(self):
        ...
        print(f"\n[SUKCES] Cały raport zapisany w folderze: {
